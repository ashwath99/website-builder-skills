...
owns:
  ...
  - Component content slots (what goes inside each component)
  - Component configuration variants
  - Component selection logic (which component for which content)
  - Component responsive behavior rules
  - Component composition rules (nesting, grouping)
requires:
  - workflow
  - design_guide
depends_on:
...
→ For token values applied to components: see `design_guide.md`
→ For HTML markup patterns: see `html_structure.md`
→ For CSS implementation rules: see `css_js_rules.md`
→ For how components are arranged into page layouts: see `layout_patterns.md`
...
→ For tab interaction JS pattern: see `css_js_rules.md`
...
→ For accordion interaction JS pattern: see `css_js_rules.md`
...
→ For sticky bar JS pattern: see `css_js_rules.md`
...
This table connects section types from `content_brief.md` to available components:
...
→ For which component variants the variation axes select: see `variation_generator.md`
