...
description: Defines page-level layout patterns including section sequencing, grid systems, content width rules, tinted section alternation, and pattern selection logic. Use when arranging sections into a full page layout, selecting a layout pattern for a brief, or implementing grid and spacing systems.
version: "5.0"
---
...
→ For individual component specs: see `component-library`
→ For token values (spacing, widths): see `design-tokens`
→ For HTML section markup: see `html-generator`
→ For variation axis options that modify layout: see `variation-explorer`
...
→ For exact responsive token adjustments: see `css-js-generator`
...
**Tint assignment:** Tints from `design-tokens` are assigned in order. If the page has more tinted sections than available tint pairs, cycle back to tint-1.
...
If `variation-explorer` has produced a Variation Spec and a variant is selected, the variant's **Narrative Flow** axis overrides the pattern selection. The variant specifies the section sequence directly.

→ For variation flow options: see `variation-explorer` Section 2.2

### Step 4: Apply Trend Modifications (if active)

If `trend-adapter` has produced a Trend Adaptation Brief, its layout modifications may adjust:
...
→ For trend layout modifications: see `trend-adapter` Section 3.4, 3.6
...
Bento grid is a non-linear layout that replaces the traditional stacked-section approach. It's selected when the **Visual Weight Distribution** trend dimension is set to `bento` (see `trend-adapter`).
...
- Or `variation-explorer` specifies bento-grid as the feature presentation axis value
...
A common mid-page section that explains the product's process in steps. Not a standalone component in `component-library` but a layout pattern for combining simple elements.
